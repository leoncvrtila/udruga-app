...
class Presidency extends Component {
    ...
    state={
        members: [],
        titles:[                                                            // naslovi vrijednosti unutar tablice
            {name: 'Ime'},
            {name: 'Prezime'},
            {name: 'Datum rođenja'},
       ...
       presidencyValue: 'Član predsjedništva',                            // prema tome se filtriraju
       subject: '',
       email: '',
       msg: '',
       eventDate: '',
    ...
    render() {
        ...
        let filteredMembers = this.state.members.filter(                        // u toj varijabli se nalaze filtrirani clanovi 
                (member) => {
                    return  member.presidency.toLowerCase().indexOf(this.state.presidencyValue.toLowerCase()) !== -1 && 
...
export default connect(mapStateToProps)(Presidency);