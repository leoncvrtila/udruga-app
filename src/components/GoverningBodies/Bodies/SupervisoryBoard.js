...
class SupervisoryBoard extends Component {

    state={
        members: [],    
        titles:[                                                                            // naslvoni vrijednosti unutar tablice
            {name: 'Ime'},  
            {name: 'Prezime'},
            {name: 'Datum rođenja'},
       ...
       supervisoryBoardValue: 'Član nadzornog odbora',                                     // prema tom se filtriraju
       subject: '',
       email: '',
       msg: '',
       eventDate: '',
    ...
    render() {
        ...
        let filteredMembers = this.state.members.filter(                            // u toj varijabli se nalaze filtrirani clanovi 
                (member) => {
                    return  member.supervisoryBoard.toLowerCase().indexOf(this.state.supervisoryBoardValue.toLowerCase()) !== -1 && 
...
export default connect(mapStateToProps)(SupervisoryBoard);