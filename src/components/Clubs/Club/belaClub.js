...
class BelaClub extends Component {
    ...
    state={
       ...
       clubValue: 'bela',                                           // prema tome se filtriraju
    ...
    render() {
        ...
        let filteredMembers = this.state.members.filter(    // u toj varijabli se nalaze filtrirani clanovi 
                (member) => {
                    return  member.bela.toLowerCase().indexOf(this.state.clubValue.toLowerCase()) !== -1 
            }
        )
...
export default connect(mapStateToProps)(BelaClub);