...
class RibolovClub extends Component {
    ...
    state={
       ...
       clubValue: 'ribolov',                                                                    // prema tome se filtriraju
       subject: '',
       email: '',
       msg: '',
       eventDate: '',
    ...
    render() {
        ...
        let filteredMembers = this.state.members.filter(                                    // u toj varijabli se nalaze filtrirani clanovi 
                (member) => {
                    return  member.ribolov.toLowerCase().indexOf(this.state.clubValue.toLowerCase()) !== -1
            }
        )
...
export default connect(mapStateToProps)(RibolovClub);