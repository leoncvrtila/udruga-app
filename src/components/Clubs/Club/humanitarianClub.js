...
class HumanitarianClub extends Component {

    state={
        members: [],
        titles:[
            {name: 'Ime'},                                           // table titles
            {name: 'Prezime'},
            {name: 'Datum rođenja'},
       ...
       clubValue: 'humanitarniRad',                                  // prema tome se filtriraju
       subject: '',                                                 // za sljanje maila
       email: '',
       msg: '',
       eventDate: '',
       eventTitle: '',
    ...
    eventHandler = () => {
        ...
        if(
            ...
            .catch(error => {
                this.setState({error: true});             // modal error
            });  
    ...
    render() {
        ...
        let filteredMembers = this.state.members.filter(    // u toj varijabli se nalaze filtrirani clanovi 
                (member) => {
                    return  member.humanitarian.toLowerCase().indexOf(this.state.clubValue.toLowerCase()) !== -1 
            }
        )
...
export default connect(mapStateToProps)(HumanitarianClub);